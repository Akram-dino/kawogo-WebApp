// Configuration
const BACKEND_API = "http://localhost:5000/api/analyze";
...
// Handle click on upload area - FIXED VERSION (NO DUPLICATES)
...
// Function to create pesticide HTML
function createPesticideHTML(pesticideData) {
  if (!pesticideData || !pesticideData.pesticides || pesticideData.pesticides.length === 0) {
    return `
  ...
  let html = `
    <div class="pesticide-section">
      <h4>🧪 Recommended Pesticides for ${pesticideData.disease}</h4>
  `;

  pesticideData.pesticides.forEach((pesticide, index) => {
    html += `
      <div class="pesticide-card">
        <div class="pesticide-header">
          <h5>${pesticide.name}</h5>
          <span class="pesticide-type">${pesticide.type}</span>
        </div>
        <div class="pesticide-details">
          <p><strong>Dosage:</strong> ${pesticide.dosage}</p>
          <p><strong>Application:</strong> ${pesticide.application}</p>
          <p class="pesticide-warning"><strong>⚠️ Precautions:</strong> ${pesticide.precautions}</p>
        </div>
      </div>
    `;
  });

  if (pesticideData.note) {
    html += `<p class="pesticide-note"><strong>Note:</strong> ${pesticideData.note}</p>`;
  }
...
async function analyzeImage() {
    ...
    // Hide pesticides section if it exists
    const existingPesticides = document.getElementById('pesticides');
    if (existingPesticides) {
        existingPesticides.style.display = 'none';
    }
    ...
    try {
        ...
        const { disease, confidence, advice, pesticides } = await response.json();
        ...
        let pesticidesDiv = document.getElementById('pesticides');
        if (!pesticidesDiv) {
            pesticidesDiv = document.createElement('div');
            pesticidesDiv.id = 'pesticides';
            pesticidesDiv.className = 'result-section';
            document.getElementById('advice').parentNode.insertBefore(pesticidesDiv, document.getElementById('advice').nextSibling);
        }
        
        pesticidesDiv.innerHTML = createPesticideHTML(pesticides);
        pesticidesDiv.style.display = 'block';
